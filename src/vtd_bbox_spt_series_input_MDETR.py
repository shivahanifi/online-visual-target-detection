...
class AttentiveObjectDetection(yarp.RFModule):
    def configure(self, rf):
        ...
        # Output port for modulated heatmap (norm_map)/ MDETR
        self.out_port_norm_map = yarp.Port()
        self.out_port_norm_map.open('/vtd_bbox/normMap:o')
        self.out_buf_norm_map_array = np.ones((IMAGE_HEIGHT, IMAGE_WIDTH, 3), dtype=np.uint8)
        self.out_buf_norm_map = yarp.ImageRgb()
        self.out_buf_norm_map.resize(IMAGE_WIDTH, IMAGE_HEIGHT)
        self.out_buf_norm_map.setExternal(self.out_buf_norm_map_array.data, self.out_buf_norm_map_array.shape[1],
                                             self.out_buf_norm_map_array.shape[0])
        print('{:s} opened'.format('/vtd_bbox/normMap:o'))
                        ...
                        if poses:
                            ...
                            with torch.no_grad():
                                   ...
                                   # prev_hx = (hx[0][:, :min(X_pad_sizes_slice[0], 1), :, :, :].detach(), hx[1][:, :min(X_pad_sizes_slice[0], 1), :, :, :].detach())
                                    ...
                                    tmp = int(time.time())
                                    raw_hm_path = "/RUN/raw_hm_dump/tensor{}.pt".format(tmp)
                                    torch.save(raw_hm, raw_hm_path)
...
#                                    cv2.waitKey(0)
                                    ...
                                    #Norm map visualization
                                    norm_map_array = np.asarray(cv2.cvtColor(norm_map, cv2.COLOR_GRAY2BGR))
                                    self.out_buf_norm_map_array[:, :] = norm_map_array
                                    self.out_port_norm_map.write(self.out_buf_norm_map)                            
