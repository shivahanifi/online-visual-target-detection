...
#                try:
                    ...
                    if poses:
                        ...
                        print('head_box : {}'.format(head_box))
                        ...
                        with torch.no_grad():
                                ...
                                head = test_transforms(head) # transform input
                                ...
                                head_channel = imutils.get_head_box_channel(head_box[0], head_box[1], head_box[2], head_box[3], width, height,
                                                                            resolution=input_resolution, coordconv=False).unsqueeze(0)
                                print('head_chanel type:',head_channel.type())

                                head = head.unsqueeze(0).cuda()
                                print('head unsqueezed size: ', head.size())
                                frame = frame.unsqueeze(0).cuda()
                                print('frame unsqueeze size: ', frame.size())
                                ...
                                lengths = [1]
                                chunk_size = 1
                                X_pad_data_img , X_pad_sizes = pack_padded_sequence(images, lengths, batch_first=False)
                                X_pad_data_head, _ = pack_padded_sequence(head_channels, lengths, batch_first=False)
                                X_pad_data_face, _ = pack_padded_sequence(heads, lengths, batch_first=False)
                                
                                X_pad_data_slice_img = X_pad_data_img[0].cuda()
                                print('X_pad_data_slice_img szie: ', X_pad_data_slice_img.size())
                                X_pad_data_slice_head = X_pad_data_head[0].cuda()
                                print('X_pad_data_slice_head size: ', X_pad_data_slice_head)
                                X_pad_data_slice_face = X_pad_data_face[0].cuda()
                                print('X_pad_data_slice_face: ', X_pad_data_slice_face)
                                ...
                                # Contour drawing
                                print("fram_raw as a numpy array has the shape ", np.asarray(X_pad_data_slice_img).shape, "and the type ", np.asarray(X_pad_data_slice_img).dtype)
